...
public class SindiceQueryEndpoint {
    ...
    public boolean hasResult(Model model, String queryString,
            ImmutableList<String> searchPropertyUris, int limit) {
        // reconcile then get types for each element in the reconciliation result assure that order
        // is preserved
        ...
        List<ReconciliationCandidate> candidates =
                reconcileEntities(queryExecutor, request, searchPropertyUris, 2.0);
        return !candidates.isEmpty();
    }

    public LinkedHashMultimap<String, String> guessType(Model model, String queryString,
            ImmutableList<String> searchPropertyUris, int limit) {
        // reconcile then get types for each element in the reconciliation result assure that order
        // is preserved
        ReconciliationRequest request = new ReconciliationRequest(queryString, limit);
    ...
    private List<ReconciliationCandidate> reconcileEntities(QueryExecutor queryExecutor,
            ReconciliationRequest request, ImmutableList<String> searchPropertyUris,
