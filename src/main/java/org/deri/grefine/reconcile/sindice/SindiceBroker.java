...
public class SindiceBroker {
    ...
    public List<String> guessDomain(String query, int limit, GRefineJsonUtilities jsonUtilities) {
        ...
        try {
            LinkedHashSet<String[]> urlPairs =
                    getUrlsForSimpleTermSearch(query, GUESS_DOMAIN_SEARCH_LIMIT, jsonUtilities);
    ...
    public Model getModelForUrl(String url, String cacheUrl, GRefineJsonUtilities jsonUtilities)
            throws JSONException, IOException {
        JSONObject sindiceCacheObj = jsonUtilities.getJSONObjectFromUrl(new URL(cacheUrl));
        JSONArray rdfContentArr =
                sindiceCacheObj.getJSONObject(url).getJSONArray("explicit_content");
    ...
    private String getDomainForUrl(String url, String cacheUrl, GRefineJsonUtilities jsonUtilities)
            throws JSONException, IOException {
        JSONObject sindiceCacheObj = jsonUtilities.getJSONObjectFromUrl(new URL(cacheUrl));
    ...
    /**
     * @param q
     * @return LinkedHashSet of string-pairs. each pair is (in order) URL of the document matching
     *         the search for q and the Sindice cache URL for the document. the LinkedHashSet
     *         assures no duplication while keeping the order of addition
     * @throws IOException
     * @throws JSONException
     */
    public LinkedHashSet<String[]> getUrlsForSimpleTermSearch(String q, int searchLimit,
            GRefineJsonUtilities jsonUtilities) throws JSONException, IOException {
        return getUrlsForSimpleTermSearch(q, null, null, searchLimit, jsonUtilities);
    }

    public LinkedHashSet<String[]> getUrlsForSimpleTermSearch(String q, String domain, String type,
            int searchLimit, GRefineJsonUtilities jsonUtilities) throws JSONException, IOException {
    ...
    private URL buildUrl(String q, String domain, String type, int searchLimit)
        ...
        try {
            ...
            String domainFilter =
                    domain == null ? "" : "domain:" + URLEncoder.encode(domain, "UTF-8") + "%20";

            String fq = domainFilter + typeFilter;
            // FIXME couldn't figure out how to tell Sindice how many items I want (itemsPerPage)
            String query = String.format("q=%s&qt=%s&fq=%s", URLEncoder.encode(q, "UTF-8"),
                    URLEncoder.encode("term", "UTF-8"), fq);
