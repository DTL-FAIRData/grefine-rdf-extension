...
public class ResultSetWrappingUtil {
    ...
    public static ImmutableList<SearchResultItem> resultSetToSearchResultListFilterDuplicates(
     ...
     * @return LinkedHashMultimap keeps the order of key input, so order in the result set is
     *         retained the first variable in resultSet solutions is the key to the map ,the second
     *         is the value
     */
    public static LinkedHashMultimap<String, String> resultSetToMultimap(ResultSet resultSet) {
        ...
        if (varNames.size() != 2) {
            throw new RuntimeException(
                    "resultSetToMultimap only accepts a resultset with exactly two variables in the solution");
        }
        ...
        while (resultSet.hasNext()) {
            ...
            String val = valNode.canAs(Resource.class) ? valNode.as(Resource.class).getURI()
                    : valNode.asLiteral().getString();
            /*
             * if(map.keySet().size()==limit && !map.keySet().contains(key)){ break; }
             */
            map.put(key, val);
        }
        return map;
    ...
    public static ImmutableList<String[]> resultSetToListOfPairs(ResultSet resultset) {
        ...
        while (resultset.hasNext()) {
            ...
            /*
             * String o; if(object.canAs(Resource.class)){ o = object.asResource().getURI(); }else{
