...
public class PlainSparqlQueryFactory extends AbstractSparqlQueryFactory {
    ...
    @Override
    public List<ReconciliationCandidate> wrapReconciliationResultset(ResultSet resultSet,
            String queryString, ImmutableList<String> searchPropertyUris, int limit,
        ...
        while (resultSet.hasNext()) {
            ...
            ReconciliationCandidate candidate = new ReconciliationCandidate(entity.getURI(),
                    scoredLabel.label, new String[] {}, scoredLabel.score, match);
    ...
    public String getExactMatchReconciliationSparqlQuery(ReconciliationRequest request,
            ImmutableList<String> searchPropertyUris) {
    ...
    public String getTypeSuggestSparqlQuery(String prefix, int limit) {
        return SUGGEST_TYPE_QUERY_TEMPLATE.replace("[[QUERY]]", prepareQueryRE(prefix))
    ...
    public List<ReconciliationCandidate> wrapResultset(ResultSet resultSet, String queryString,
            double matchThreshold) {
        List<ReconciliationCandidate> candidates = new ArrayList<ReconciliationCandidate>();
        while (resultSet.hasNext()) {
            QuerySolution solution = resultSet.nextSolution();
            Resource entity = solution.getResource("entity");
            ReconciliationCandidate candidate = new ReconciliationCandidate(entity.getURI(),
                    queryString, new String[] {}, 1.0d, true);
    ...
    public String getPropertySuggestSparqlQuery(String prefix, String typeUri, int limit) {
        return SUGGEST_PROPERTY_WITH_SPECIFIC_SUBJECT_TYPE_QUERY_TEMPLATE
                .replace("[[QUERY]]", prefix).replace("[[TYPE_URI]]", typeUri)
                .replace("[[LIMIT]]", String.valueOf(limit));
    }

    @Override
    public String getPropertySuggestSparqlQuery(String prefix, int limit) {
        return SUGGEST_PROPERTY_QUERY_TEMPLATE.replace("[[QUERY]]", prefix).replace("[[LIMIT]]",
    ...
    public String getEntitySearchSparqlQuery(String prefix,
        ...
        for (int i = 1; i <= searchPropertyUris.size(); i++) {
            ...
            regexPatternClause
                    .append(REGEX_SEARCH_PATTERN.replaceAll("\\[\\[INDEX\\]\\]", String.valueOf(i))
                            .replace("[[QUERY]]", prefix).replace("[[PROPERTY_URI]]", propUri));
     ...
     */
    private ScoredLabel getPreferredLabel(QuerySolution solution, String queryString,
            ImmutableList<String> searchPropertyUris) {
        double maxScore = -1d;
        String bestLabel = "";
        for (int i = 1; i <= searchPropertyUris.size(); i++) {
        ...
        if (bestLabel.isEmpty()) {
            // fail... should never get here as bound restrictions are added to the SPARQL query
            // used
    ...
    private String getFulltextFilter(String queryString, ImmutableList<String> searchPropertyUris) {
        ...
        for (int i = 1; i <= searchPropertyUris.size(); i++) {
            String propUri = searchPropertyUris.get(i - 1);
            regexPatternClause.append(REGEX_SEARCH_PATTERN
                    .replaceAll("\\[\\[INDEX\\]\\]", String.valueOf(i))
                    .replace("[[QUERY]]", queryString).replace("[[PROPERTY_URI]]", propUri));
            boundFilterClause.append(BOUND_LABEL_FILTER.replace("[[INDEX]]", String.valueOf(i)));
        }
        // remove the last additional " ||"
        boundFilterClause.setLength(boundFilterClause.length() - 3);

        return FULLTEXT_SEARCH_FILTER
                .replace("[[REGEX_SEARCH_PATTERN]]", regexPatternClause.toString())
                .replace("[[BOUND_LABEL_FILTER]]", boundFilterClause.toString());
    }

    private String getExactMatchFulltextFilter(String queryString,
            ImmutableList<String> searchPropertyUris) {
        if (searchPropertyUris.size() == 1) {
            // simple optimization
        ...
        for (int i = 1; i <= searchPropertyUris.size(); i++) {
            String propUri = searchPropertyUris.get(i - 1);
            exactMatchPatternClause.append(EXACT_MATCH_SEARCH_PATTERN
                    .replace("[[QUERY]]", queryString).replace("[[PROPERTY_URI]]", propUri)
                    .replaceAll("\\[\\[INDEX\\]\\]", String.valueOf(i)));
        }
        // remove the last additional "UNION "
        ...
        return EXACT_MATCH_FULLTEXT_SEARCH_FILTER.replace("[[EXACT_MATCH_SEARCH_PATTERN]]",
                exactMatchPatternClause.toString());
    }
    ...
    private static final String SUGGEST_TYPE_QUERY_TEMPLATE = "SELECT DISTINCT ?type ?label "
            + "WHERE{" + "[] a ?type. " + "?type ?p ?label. "
            + "FILTER (?p=<http://www.w3.org/2000/01/rdf-schema#label> || ?p=<http://www.w3.org/2004/02/skos/core#prefLabel>). "
            + "FILTER regex(str(?label),'^[[QUERY]]','i')" + "} LIMIT [[LIMIT]]";

    private static final String SUGGEST_PROPERTY_WITH_SPECIFIC_SUBJECT_TYPE_QUERY_TEMPLATE =
            "SELECT DISTINCT ?p ?label " + "WHERE{" + "[] a <[[TYPE_URI]]>;" + "?p ?v."
                    + "?p ?label_prop ?label. "
                    + "FILTER (?label_prop=<http://www.w3.org/2000/01/rdf-schema#label> || ?label_prop=<http://www.w3.org/2004/02/skos/core#prefLabel>). "
                    + "FILTER regex(str(?label),'^[[QUERY]]','i')" + "} LIMIT [[LIMIT]]";
    private static final String SUGGEST_PROPERTY_QUERY_TEMPLATE = "SELECT DISTINCT ?p ?label "
            + "WHERE{" + "[] ?p ?v." + "?p ?label_prop ?label. "
            + "FILTER (?label_prop=<http://www.w3.org/2000/01/rdf-schema#label> || ?label_prop=<http://www.w3.org/2004/02/skos/core#prefLabel>). "
            + "FILTER regex(str(?label),'^[[QUERY]]','i')" + "} LIMIT [[LIMIT]]";

    private static final String SEARCH_ENTITY_QUERY_TEMPLATE = "SELECT ?entity[[LABEL_CLAUSE]] "
            + "WHERE" + "{" + "[[REGEX_SEARCH_PATTERN]]" + "FILTER ([[BOUND_LABEL_FILTER]]). "
            + "FILTER isIRI(?entity). }LIMIT [[LIMIT]]";
}
