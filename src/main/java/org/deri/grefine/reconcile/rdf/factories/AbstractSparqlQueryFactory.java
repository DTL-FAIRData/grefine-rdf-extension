...
/**
 * @author fadmaa provides a default implementation of
 *         {@link org.deri.grefine.reconcile.rdf.factories.SparqlQueryFactory} this implementation
 *         takes care of the well-defined parts of the queries a.k.a standardised while leave the
 *         rest to actual implementation through abstract methods.
 * 
 */
public abstract class AbstractSparqlQueryFactory implements SparqlQueryFactory {
    ...
    public String getSampleInstancesSparqlQuery(String typeUri,
        ...
        for (String propUri : searchPropertyUris) {
            labelPropertyClause.append(LABEL_PROPERTY_FILTER.replace("[[PROPERTY_URI]]", propUri)
    ...
    public String getResourcePropertiesMapSparqlQuery(String resourceId, int limit) {
        return RESOURCE_PROPERTIES_QUERY.replace("[[RESOURCE]]", resourceId).replace("[[LIMIT]]",
    ...
    @Override
    public ImmutableList<SearchResultItem> wrapEntitySearchResultSet(ResultSet resultSet,
            int limit) {
        return ResultSetWrappingUtil.resultSetToSearchResultListFilterDuplicates(resultSet, limit);
    }

    @Override
    public ImmutableList<SearchResultItem> wrapSampleInstancesResultSet(ResultSet resultSet,
    ...
    private String getFirstNonNullLabel(QuerySolution sol,
    ...
    @Override
    public ImmutableList<String[]> wrapSampleValuesOfPropertyResultSet(ResultSet resultSet,
    ...
    @Override
    public String getResourcePropertiesMapSparqlQuery(PreviewResourceCannedQuery cannedQuery,
            String resourceId) {
    ...
    public Multimap<String, String> wrapResourcePropertiesMapResultSet(
            PreviewResourceCannedQuery cannedQuery, ResultSet resultset) {
        return cannedQuery.wrapResourcePropertiesMapResultSet(resultset);
    }

    /**
     * put the ResultSet returned from SPARQL endpoint into the
     * {@link org.deri.grefine.reconciliation.model.GRefineReconciliationResponse
     * GRefineReconciliationResponse} <br/>
     * 
     * @param result
     * @return
     */
    @Override
    public List<ReconciliationCandidate> wrapReconciliationResultset(ResultSet result,
        ...
        while (result.hasNext()) {
            ...
            // they cannot be used as percentage see:
            ...
            double scoreWeight =
                    scoreWieghtLiteral == null ? 1 : scoreWieghtLiteral.getDouble() / maxScore;
            ...
            candidates.add(
                    new ReconciliationCandidate(entityUri, label, new String[] {}, score, match));
    ...
    private static final String SAMPLE_INSTANCES_QUERY =
            "SELECT DISTINCT ?entity [[LABEL_CLAUSE]] " + "WHERE{" + "?entity a <[[TYPE_URI]]>. "
                    + "[[LABEL_PROPERTY_CLAUSE]]" + "}LIMIT [[LIMIT]]";

    private static final String SAMPLE_PROPERTY_INSTANCES_QUERY =
            "PREFIX rdfs:<http://www.w3.org/2000/01/rdf-schema#> " + "SELECT DISTINCT ?s ?o "
                    + "WHERE{" + "?s <[[PROPERTY_URI]]> ?o. " + "}LIMIT [[LIMIT]]";

    private static final String RESOURCE_PROPERTIES_QUERY =
            "SELECT DISTINCT ?p ?v " + "WHERE{ " + "<[[RESOURCE]]> ?p ?v. " + "}LIMIT [[LIMIT]]";

    private static final String TYPES_OF_ENTITIES_QUERY = "SELECT ?entity ?type " + "WHERE{ "
            + "?entity a ?type. " + "FILTER ([[ENTITY_EQUALITY_FILTER]]). " + "}";
    private static final String LABEL = " ?label";
    private static final String LABEL_PROPERTY_FILTER =
