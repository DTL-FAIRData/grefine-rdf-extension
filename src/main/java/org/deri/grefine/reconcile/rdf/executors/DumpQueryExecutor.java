...
/**
 * @author fadmaa execute SPARQL queries agains Dump RDF and supports LARQ for full text searc as
 *         index in built with the model this calss can be costly to build consider sharing
 *         instances of this class It is thread-safe
 */
public class DumpQueryExecutor implements QueryExecutor {
    ...
    public DumpQueryExecutor(Model m, String propertyUri, boolean ngramIndex, int minGram,
            int maxGram) {
        loaded = true;
        this.propertyUri = propertyUri;
    ...
    public synchronized void initialize(FileInputStream in) {
        ...
        EntityDefinition entDef =
                new EntityDefinition("uri", "text", model.getResource(propertyUri));
