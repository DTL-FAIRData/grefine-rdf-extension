...
public class PlainSparqlQueryEndpoint extends QueryEndpointImpl {
    ...
    public PlainSparqlQueryEndpoint(PlainSparqlQueryFactory queryFactory,
     ...
     * (non-Javadoc)
     * 
     * @see org.deri.grefine.reconcile.rdf.endpoints.QueryEndpointImpl#reconcileEntities(com.google.
     * refine.com.google.refine.org.deri.reconcile.model.ReconciliationRequest,
     * com.google.common.collect.ImmutableList)
     * 
     * this method will try first to search for exact match...if it succeeds the return results. if
     * not, it resorts to the more costly method define in its superclass
     */
    @Override
    public List<ReconciliationCandidate> reconcileEntities(ReconciliationRequest request,
        ...
        String sparql = plainQueryFactory.getExactMatchReconciliationSparqlQuery(request,
                searchPropertyUris);
        ResultSet resultSet = queryExecutor.sparql(sparql);
        List<ReconciliationCandidate> candidates = plainQueryFactory.wrapResultset(resultSet,
                request.getQueryString(), matchThreshold);
