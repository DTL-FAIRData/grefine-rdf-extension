...
public class PreviewResourceCannedQuery {
    ...
    public PreviewResourceCannedQuery(InputStream properties) throws IOException {
        ...
        for (int i = 0; i < labels.length; i++) {
            ...
            whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri).append("> ?en_label")
                    .append(i).append(" FILTER langMatches(lang(?en_label").append(i)
                    .append("),'en') } ");
            whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri).append("> ?label")
                    .append(i).append(" FILTER langMatches(lang(?label").append(i)
                    .append("),'') } ");
        }
        ...
        for (int i = 0; i < descriptions.length; i++) {
            ...
            whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri).append("> ?en_desc")
                    .append(i).append(" FILTER langMatches(lang(?en_desc").append(i)
                    .append("),'en') } ");
            whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri).append("> ?desc")
                    .append(i).append(" FILTER langMatches(lang(?desc").append(i)
                    .append("),'') } ");
        }
        ...
        for (int i = 0; i < images.length; i++) {
            ...
            whereClause.append("OPTIONAL { <[[RESOURCE_URI]]> <").append(uri).append("> ?img")
                    .append(i).append("} ");
        }
    ...
    private List<String> getFirstBoundN(QuerySolution sol, List<String> varNames, String varName,
            int num, boolean langAware) {
