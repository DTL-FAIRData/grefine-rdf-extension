...
public class QueryEndpointImpl implements QueryEndpoint {
    ...
    @Override
    public List<ReconciliationCandidate> reconcileEntities(ReconciliationRequest request,
            ImmutableList<String> searchPropertyUris, double matchThreshold) {
        ...
        List<ReconciliationCandidate> candidates =
                this.queryFactory.wrapReconciliationResultset(resultSet, request.getQueryString(),
                        searchPropertyUris, request.getLimit(), matchThreshold);
        ...
        if (request.getTypes().length == 0 && candidates.size() > 0) {
            ...
            String typeSparql =
                    this.queryFactory.getTypesOfEntitiesQuery(ImmutableList.copyOf(entities));
            ResultSet typeResultSet = this.queryExecutor.sparql(typeSparql);
            Multimap<String, String> typesMap =
                    this.queryFactory.wrapTypesOfEntities(typeResultSet);
            for (ReconciliationCandidate candidate : candidates) {
    ...
    @Override
    public ImmutableList<SearchResultItem> suggestProperty(String prefix, String typeUri,
            int limit) {
    ...
    @Override
    public ImmutableList<SearchResultItem> getSampleInstances(String typeUri,
            ImmutableList<String> searchPropertyUris, int limit) {
        String sparql =
                this.queryFactory.getSampleInstancesSparqlQuery(typeUri, searchPropertyUris, limit);
        ResultSet resultSet = this.queryExecutor.sparql(sparql);
        return this.queryFactory.wrapSampleInstancesResultSet(resultSet, typeUri,
                searchPropertyUris, limit);
    }
    ...
    @Override
    public Multimap<String, String> getResourcePropertiesMap(PreviewResourceCannedQuery cannedQuery,
            String resourceUri) {
        String sparql =
                this.queryFactory.getResourcePropertiesMapSparqlQuery(cannedQuery, resourceUri);
        ResultSet resultSet = this.queryExecutor.sparql(sparql);
        return this.queryFactory.wrapResourcePropertiesMapResultSet(cannedQuery, resultSet);
    }

    @Override
    public ImmutableList<SearchResultItem> searchForEntities(String prefix,
            ImmutableList<String> searchPropertyUris, int limit) {
        String sparql =
                this.queryFactory.getEntitySearchSparqlQuery(prefix, searchPropertyUris, limit);
        ResultSet resultSet = this.queryExecutor.sparql(sparql);
