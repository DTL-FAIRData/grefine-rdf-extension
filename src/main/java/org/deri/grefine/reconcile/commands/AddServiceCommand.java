...
public class AddServiceCommand extends AbstractAddServiceCommand {
    ...
    protected ReconciliationService getReconciliationService(HttpServletRequest request)
        ...
        if (name.isEmpty() || url.isEmpty() || propUris.size() == 0) {
            throw new RuntimeException(
                    "name, endpoint URL and at least one label property ar needed");
        }
    ...
    private ReconciliationService getRdfService(String name, String id, String url, String format,
            ImmutableList<String> propUris, HttpServletRequest request) {
    ...
    private ReconciliationService getSparqlService(String name, String id, String url,
            ImmutableList<String> propUris, HttpServletRequest request) {
        ...
        ReconciliationService service = new RdfReconciliationService(id, name, propUris,
                queryEndpoint, DEFAULT_MATCH_THRESHOLD);
        return service;
    }
}
