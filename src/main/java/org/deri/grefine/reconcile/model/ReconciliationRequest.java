...
public class ReconciliationRequest {
    ...
    /**
     * the limit used in SPARQL query when types are restricted i.e. the query asks for matches of
     * one of specified types
     */
    private static final int DEFAULT_LIMIT = 3;

    /**
     * the limit used in SPARQL query when types are <b>not</b> restricted. More results is
     * typically needed in this case and this is why MAXIMUM_LIMIT > DEFAULT_LIMIT
     */
    ...
    public static ReconciliationRequest valueOf(String json)
        ...
        // get limit
        int limit = node.path("limit").isMissingNode() ? getDefaultLimit(types.length)
                : node.path("limit").getIntValue();
        // get type_strict
        ...
        if (propertiesNode.isArray()) {
            ...
            for (int i = 0; i < propertiesArray.size(); i += 1) {
                ...
                if (valueNodeId.isMissingNode()) {
                    // textual value
                    value = new ReconciliationRequestContext.TextualValueContext(
                            valueNode.getTextValue());
                } else {
                    // identified value
                    value = new ReconciliationRequestContext.IdentifiedValueContext(
                            valueNodeId.getTextValue());
                }
    ...
    public boolean equals(Object obj) {
        ...
        return this.queryString.equals(otherReq.getQueryString())
                && this.getLimit() == otherReq.limit && this.getTypes().equals(otherReq.getTypes());
    }

    /**
     * when types are restricted (i.e. typesLength>0) the query is specific and the default limit
     * needs to be small thus we use DEFAULT_LIMIT when types are not restricted (i.e.
     * typesLength==0) the query is more of a an explorative nature and needs to see what types
     * exist for what we are looking for and thus we use the MAXIMUM_LIMIT
     * 
     * @param typesLength
     * @return
     */
