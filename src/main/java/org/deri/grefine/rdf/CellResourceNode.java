...
public class CellResourceNode extends ResourceNode implements CellNode {
    ...
    @Override
    public Resource[] createResource(URI baseUri, ValueFactory factory, Project project, Row row,
            int rowIndex, BNode[] blanks) {
        try {
            Object result =
                    Util.evaluateExpression(project, uriExpression, columnName, row, rowIndex);
            ...
            // an empty cell might result in an exception out of evaluating URI expression... so it
            // is intended to eat the exception
            return null;
    ...
    protected void writeNode(JSONWriter writer) throws JSONException {
        writer.key("nodeType");
        writer.value("cell-as-resource");
        writer.key("expression");
        writer.value(uriExpression);
        writer.key("isRowNumberCell");
        writer.value(isRowNumberCell);
        if (columnName != null) {
