...
public class RdfSchema implements OverlayModel {
    ...
    public void dispose(Project project) {
        /*
         * try {
         * ApplicationContext.instance().getVocabularySearcher().deleteProjectVocabularies(String.
         * valueOf(project.id)); } catch (ParseException e) { //log
         * logger.error("Unable to delete index for project " + project.id, e); } catch (IOException
         * e) { //log logger.error("Unable to delete index for project " + project.id, e); }
         */
    }
    ...
    public RdfSchema(ApplicationContext ctxt, Project project)
        ...
        if (this.prefixesMap == null || this.prefixesMap.isEmpty()) {
            this.prefixesMap =
                    clone(ctxt.getPredefinedVocabularyManager().getPredefinedVocabulariesMap());
            // copy the index of predefined vocabularies
            // each project will have its own copy of these predefined vocabs to enable delete,
            // update ....
    ...
    static protected Node reconstructNode(JSONObject o, RdfSchema s) throws JSONException {
        Node node = null;
        ...
        if (nodeType.startsWith("cell-as-")) {
            ...
            if ("cell-as-resource".equals(nodeType)) {
                ...
                String valueType = o.has("valueType")
                        ? Util.getDataType(s.getBaseUri(), o.getString("valueType"))
                        : null;
            ...
            String valueType =
                    o.has("valueType") ? Util.getDataType(s.getBaseUri(), o.getString("valueType"))
                            : null;
            String lang = o.has("lang") ? o.getString("lang") : null;
            // strip off @
            lang = stripAtt(lang);
        ...
        if (node != null && node instanceof ResourceNode && o.has("links")) {
            ...
            for (int j = 0; j < linkCount; j++) {
                ...
                node2.addLink(new Link(oLink.getString("uri"), oLink.getString("curie"),
                        oLink.has("target") && !oLink.isNull("target")
                                ? reconstructNode(oLink.getJSONObject("target"), s)
                                : null));
            }
        }
    ...
    static private void reconstructTypes(ResourceNode node, JSONObject o) throws JSONException {
    ...
    private Map<String, Vocabulary> clone(Map<String, Vocabulary> original) {
        ...
        for (Entry<String, Vocabulary> entry : original.entrySet()) {
            copy.put(entry.getKey(),
                    new Vocabulary(entry.getValue().getName(), entry.getValue().getUri()));
        }
