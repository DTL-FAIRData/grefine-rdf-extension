...
public class SaveRdfSchemaCommand extends RdfCommand {
    ...
    public void doPost(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
             ...
             * "application/json");
             * 
             * Writer w = response.getWriter(); JSONWriter writer = new JSONWriter(w);
             * writer.object(); writer.key("code"); writer.value("ok"); writer.key("historyEntry");
             * 
             * //dummy history for now writer.object(); writer.key("op");
             * writer.value("saveRdfSchema"); writer.key("description");
             * writer.value("Save RDF schema"); writer.endObject();
             * 
             * writer.endObject();
             * 
             * w.flush(); w.close();
