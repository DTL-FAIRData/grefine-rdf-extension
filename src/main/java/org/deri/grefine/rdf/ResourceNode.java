...
abstract public class ResourceNode implements Node {
    ...
    public void write(JSONWriter writer, Properties options) throws JSONException {
        ...
        for (RdfType type : this.getTypes()) {
            writer.object();
            writer.key("uri");
            writer.value(type.uri);
            writer.key("curie");
            writer.value(type.curie);
            writer.endObject();
        }
        writer.endArray();
        // write links
    ...
    protected void addTypes(Resource[] rs, ValueFactory factory, RepositoryConnection con,
            URI baseUri) throws RepositoryException {
        for (Resource r : rs) {
            for (RdfType type : this.getTypes()) {
                Statement stmt = factory.createStatement(r, RDF.TYPE,
                        factory.createURI(Util.resolveUri(baseUri, type.uri)));
                con.add(stmt);
            }
        }
    }

    protected Resource[] addLinks(Resource[] rs, URI baseUri, ValueFactory factory,
            RepositoryConnection con, Project project, Row row, int rowIndex, BNode[] blanks)
    ...
    public abstract Resource[] createResource(URI baseUri, ValueFactory factory, Project project,
